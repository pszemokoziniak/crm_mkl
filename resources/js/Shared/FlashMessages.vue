<template>
  <div>
    ...
    <div v-if="$page.props.flash.warning && show" class="flex items-center justify-between mb-8 max-w-3xl bg-orange-500 rounded-sm">
      <div class="flex items-center">
        <svg class="shrink-0 ml-4 mr-2 w-4 h-4 fill-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M10 1L0 19h20L10 1zm0 5l6.5 11h-13L10 6zm-1 4v4h2v-4H9zm0 5v2h2v-2H9z" /></svg>
    ...
    <div v-if="($page.props.flash.error || Object.keys($page.props.errors).length > 0) && show" class="flex items-center justify-between mb-8 max-w-3xl bg-red-500 rounded-sm">
      <div class="flex items-center">
        ...
        <div v-else class="py-4 text-white text-sm font-medium">
          <!-- Wypisywana była surowa lista kluczy pól, np. [ "email" ] —
               użytkownik widział nazwę z kodu zamiast informacji, co zrobić.
               Treść błędu i tak stoi przy właściwym polu formularza. -->
          <span>{{ opisBledow }}</span>
        </div>
      </div>
      <button type="button" class="group mr-2 p-2" @click="show = false">
...
export default {
  ...
  computed: {
    ...
    // "5 błędy" po polsku nie brzmi — odmieniamy jak w reszcie systemu.
    opisBledow() {
      const n = this.liczbaBledow
      if (n === 1) return 'Formularz ma jeden błąd — popraw zaznaczone pole.'

      const ost = n % 10
      const dwie = n % 100
      const forma = ost >= 2 && ost <= 4 && (dwie < 12 || dwie > 14) ? 'błędy' : 'błędów'

      return `Formularz ma ${n} ${forma} — popraw zaznaczone pola.`
    },
  },
  watch: {
    '$page.props.flash': {
