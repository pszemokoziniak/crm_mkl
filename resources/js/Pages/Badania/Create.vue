...
export default {
  ...
  computed: {
    podpowiedz() {
      const wpis = this.istniejace[this.form.badaniaTyp_id]

      if (!wpis) return ''

      const nazwa = (this.badanias.find((p) => p.id === Number(this.form.badaniaTyp_id)) || {}).name || 'tego rodzaju'
      const waznosc = wpis.bezterminowy ? 'bez daty końca' : `ważne do ${wpis.do}`

      return `Jest już badanie tego rodzaju: ${nazwa}, ${waznosc}. `
