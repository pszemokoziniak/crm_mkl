<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-6">
      <Link class="btn-indigo" :href="`/contacts/${contact.id}/pbioz/create`">
        <span>Dodaj</span>
      </Link>
    </div>
    <div class="bg-white rounded-md shadow overflow-x-auto">
      <table class="w-full whitespace-nowrap">
        ...
        <tr v-for="item in pbioz.data" :key="item.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          <td class="border-t">
            <Link class="flex items-center px-6 py-4 focus:text-indigo-500" :href="`/contacts/${contact.id}/pbioz/${item.id}/edit`">
              {{ item.name }}
          ...
          <td class="w-px border-t">
            <Link class="flex items-center px-4" :href="`/contacts/${contact.id}/pbioz/${item.id}/edit`" tabindex="-1">
