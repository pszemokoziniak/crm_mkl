    ...
    <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="update">
        ...
        <div class="flex items-center px-8 py-4 bg-gray-50 border-t border-gray-100">
          <button v-if="!pbioz.deleted_at" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy">Usuń</button>
...
export default {
  ...
  data() {
    return {
      ...
      form: this.$inertia.form({
        id: this.pbioz.id,
        name: this.pbioz.name,
        start: this.pbioz.start,
        end: this.pbioz.end,
      }),
    }
  },
  methods: {
    update() {
      this.form.put(`/contacts/${this.contact.id}/pbioz/${this.form.id}`)
    },
    destroy() {
      if (confirm('Chcesz usunąć?')) {
        this.$inertia.delete(`/pbioz/${this.pbioz.id}`)
      }
    },
    restore() {
