<template>
  <div>
    ...
    <h1 class="mb-8 text-3xl font-bold">
      <Link class="text-indigo-400 hover:text-indigo-600" href="/uprawnieniaTyp">Uprawnienia</Link>
      <span class="text-indigo-400 font-medium">/</span>
      {{ form.name }}
    </h1>
     <trashed-message v-if="uprawnienia.deleted_at" class="mb-6" @restore="restore">Usuniąć?</trashed-message>
...
export default {
  ...
  methods: {
    update() {
      this.form.put(`/uprawnieniaTyp/${this.uprawnienia.id}`)
    },
    destroy() {
      if (confirm('Chcesz usunąć?')) {
        this.$inertia.delete(`/uprawnieniaTyp/${this.uprawnienia.id}`)
      }
    },
    restore() {
      if (confirm('Chcesz przywrócić?')) {
        this.$inertia.put(`/uprawnieniaTyp/${this.uprawnienia.id}/restore`)
      }
    },
  },
}
