<template>
  <div>
    ...
    <h1 class="mb-8 text-3xl font-bold">
      <Link class="text-indigo-400 hover:text-indigo-600" href="/jezykTyp">Języki</Link>
      <span class="text-indigo-400 font-medium">/</span>
      {{ form.name }}
    </h1>
     <trashed-message v-if="jezyk.deleted_at" class="mb-6" @restore="restore">Usuniąć?</trashed-message>
  ...
  data() {
    return {
      form: this.$inertia.form({
        id: this.jezyk.id,
        name: this.jezyk.name,
      }),
    }
  },
  methods: {
    update() {
      this.form.put(`/jezykTyp/${this.jezyk.id}`)
    },
    destroy() {
      if (confirm('Chcesz usunąć?')) {
        this.$inertia.delete(`/jezykTyp/${this.jezyk.id}`)
      }
    },
    restore() {
      if (confirm('Chcesz przywrócić?')) {
        this.$inertia.put(`/jezykTyp/${this.jezyk.id}/restore`)
      }
    },
  },
}
