<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-6">
      <Link v-if="userOwner !== 3" class="btn-indigo" :href="`/contacts/${contact.id}/uprawnienia/create`">
        <span>Dodaj</span>
      </Link>
    </div>
    <div class="bg-white rounded-md shadow overflow-x-auto">
      <table class="w-full whitespace-nowrap">
        ...
        <tr v-for="item in uprawnienias.data" :key="item.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          <td class="border-t">
            <Link class="flex items-center px-6 py-4 focus:text-indigo-500" :href="userOwner === 3 ? '' : `/contacts/${contact.id}/uprawnienia/${item.id}/edit`">
          ...
          <td class="w-px border-t">
            <Link class="flex items-center px-4" :href="userOwner === 3 ? '' : `/contacts/${contact.id}/uprawnienia/${item.id}/edit`" tabindex="-1">
...
export default {
  ...
  methods: {
    ...
    removeDocument(documentId) {
      this.$inertia.delete(`/contacts/${this.contactId}/documents/${documentId}/uprawnienia`)
    },
  },
}
</script>
